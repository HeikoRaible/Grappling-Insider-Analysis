...
def getShinglesCharbased(text, windowLength, step):
    result = set()
    for i in range(0, len(text) - (windowLength - 1)):
        sh = text[i:i+windowLength]
        crc = binascii.crc32(sh.encode()) & 0xffffffff
        result.add(crc)
    return(result)
...
numDocs = 1000
dataFile = os.path.join("..", "daten", "min_hash_data", "articles_" + str(numDocs) + ".train")
truthFile = os.path.join("..", "daten", "min_hash_data", "articles_" + str(numDocs) + ".truth")
...
for i in range(0, numDocs):
    ...
    ## ## ##
    #del words[0]
    line = line[len(docID)+1:]

    ## ## ##
    windowLength = 5
    step = 1
    shinglesInDoc = getShinglesCharbased(line, windowLength, step)
...
for i in range(0, numDocs):
    for j in range(i + 1, numDocs):
        ...
        if estJ > threshold:
            ...
            if plagiaries.get(docNames[i]) == docNames[j]:
                tp = tp + 1
                actual.append(1)
            else:
                fp = fp + 1
